...
export async function publishAuditResults(
  ...
  // Ensure .gitignore has audit exclusion rules
  await ensureAuditGitignore(sourcePath);

  // Commit all changes
  const filesToCommit = ['.gitignore', reportFileName];
  if (testFilesCopied > 0) {
...
/**
 * Ensures .gitignore contains rules to exclude Uatu-generated audit files
 * This prevents audit artifacts from polluting the user's repository
 */
async function ensureAuditGitignore(sourcePath: string): Promise<void> {
  const gitignorePath = path.join(sourcePath, '.gitignore');

  const auditIgnoreRules = `
# Uatu Audit Generated Files
# These files are generated by the Uatu audit system and should not be committed
test/audit/              # Generated test cases
AUDIT_REPORT_*.md        # Audit reports
.uatu/                   # Uatu workspace metadata
`;
  ...
  // Check if audit rules already present
  if (existingContent.includes('# Uatu Audit Generated Files')) {
    console.log('.gitignore already contains Uatu audit rules');
    return;
  }
  ...
  const newContent = existingContent
    ...
    : auditIgnoreRules.trim();

  await fs.writeFile(gitignorePath, newContent, 'utf-8');
  console.log('.gitignore updated with Uatu audit exclusion rules');
}
